...
def properties_given_theta(alpha,Halo_subs,rvir,B,Halo_ML_prob,sigma_M,gamma_M,sigma_r,sigma_mpeak,A,n,Mhm,
                           bin_modulation,bin_lower,bin_upper,mpeak_cut,xi_8,xi_9,xi_10):
    ...
    mpeak_prob[np.logical_and(np.log10(Halo_subs['mpeak']/h)>7.5,np.log10(Halo_subs['mpeak']/h)<8.5)]*=xi_8
    mpeak_prob[np.logical_and(np.log10(Halo_subs['mpeak']/h)>8.5,np.log10(Halo_subs['mpeak']/h)<9.5)]*=xi_9
    mpeak_prob[np.logical_and(np.log10(Halo_subs['mpeak']/h)>9.5,np.log10(Halo_subs['mpeak']/h)<10.5)]*=xi_10
...
def log_prior(alpha, sigma_M, mpeak_cut, B, sigma_mpeak, A, sigma_r, n, xi_8, xi_9, xi_10):
    ...
    n_min, n_max = 0.0, 2.0
    xi_8_min, xi_8_max = 0.1, 10
    xi_9_min, xi_9_max = 0.1, 10
    xi_10_min, xi_10_max = 0.1, 10
    if (alpha > alpha_min)*(alpha < alpha_max)*(B > B_min)*(B < B_max)*(mpeak_cut > mpeak_cut_min)*(mpeak_cut < mpeak_cut_max)*(sigma_M > sigma_M_min)*(sigma_M < sigma_M_max)*(sigma_mpeak > sigma_mpeak_min)*(sigma_mpeak < sigma_mpeak_max)*(A > A_min)*(A<A_max)*(sigma_r>sigma_r_min)*(sigma_r<sigma_r_max)*(n>n_min)*(n<n_max)*(xi_8>xi_8_min)*(xi_8<xi_8_max)*(xi_9>xi_9_min)*(xi_9<xi_9_max)*(xi_10>xi_10_min)*(xi_10<xi_10_max):
...
def properties_given_theta_v3(alpha,Halo_subs,rvir,B,Halo_ML_prob,sigma_M,gamma_M,sigma_r,sigma_mpeak,A,n,Mhm,
                           bin_modulation,bin_lower,bin_upper, n_realizations,mpeak_cut,xi_8,xi_9,xi_10):
    """ Returns subhalo magnitude (Mr), half-light radii (r1/2), and disruption probability (prob) 
        given model parameters (and ...) """
    ###
    ...
    mpeak_prob[np.logical_and(np.log10(mpeak/h)>7.5,np.log10(mpeak/h)<8.5)]*=xi_8
    mpeak_prob[np.logical_and(np.log10(mpeak/h)>8.5,np.log10(mpeak/h)<9.5)]*=xi_9
    mpeak_prob[np.logical_and(np.log10(mpeak/h)>9.5,np.log10(mpeak/h)<10.5)]*=xi_10
...
def lnpost_global_data_v3(y):
    ...
    Mr, r12, probs = properties_given_theta_v3(
        ...
        total_data['Halo_ML_prob'], y[1], gamma_M, y[6],y[4],y[5],y[7],0.1,
        1.,9.5,10., n_realizations,y[2],y[8],y[9],y[10]) 
...
for i in [2]:
    ...
    for j,d in enumerate(data):
        ...
        alpha, mpeak_cut, B, sigma_M, sigma_r, A, n, sigma_mpeak, xi_8, xi_9, xi_10 = -1.436, 7.85, 0.93, 0.2, 0.63, 0.037, 1., 0.2, 1.0, 1.0, 1.0
        count1_all = []
        count2_all = []
        for k in range(n_realizations):
            Mr, r12, probs = properties_given_theta(
                    alpha, d['Halo_subs'], d['rvir'], B, d['Halo_ML_prob'], sigma_M, gamma_M, sigma_r, sigma_mpeak,A,n,0.1,
                1.,7.,1.,mpeak_cut,xi_8,xi_9,xi_10)
            mu = Mr+36.57+2.5*np.log10(2*np.pi*((r12)**2))
    ...
    with open('datavector_shmf{}.pickle'.format(i),
              'wb') as handle:
    ...
    fig = c.plotter.plot(figsize=(12,12),extents=[[-2,-1.1],[0,2.0],[5,10],[10**-5,3],[0.025,1.0],[10,500],[0,2.0],[0,2],[0.1,10],[0.1,10],[0.1,10]],
    truth=[-1.436, 0.2, 8, 0.93, 0.2, 1000.0*0.037, 0.63, 1.,1.,1.,1.],
    display=True,filename="corner_shmf{}.pdf".format(i))
