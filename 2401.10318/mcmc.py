...
hyper_params['Mhm'] = 0.1
hyper_params['bin_modulation'] = 1.
hyper_params['bin_lower'] = 1.
hyper_params['bin_upper'] = 2.
...
for i in [nhosts]:
    ...
    datavector = {}
    global data
    data = data_loader.load_halo_data(halo_numbers)
    global total_data
    total_data = data_loader.load_halo_data_total(data,halo_numbers)
    ###
    datavector = data_vector.generate_datavector(data,n_realizations_datavector,bins,true_params,hyper_params,
                                                halo_numbers)
    ...
    for move in moves:
        if MPI == True:
            with MPIPool() as pool:
                ...
                sampler = emcee.EnsembleSampler(nwalkers, ndim, posterior.lnpost_global_data_v3, moves=move, pool=pool)
                start = time.time()
            ...
            with Pool(processes=16) as pool:
                np.random.seed(93284)
                sampler = emcee.EnsembleSampler(nwalkers, ndim, posterior.lnpost_global_data_v3, moves=move, pool=pool,
                                               args=(total_data,datavector,hyper_params,n_realizations,bins,halo_numbers))
                start = time.time()
                sampler.run_mcmc(p0, n_steps, progress=True)
                end = time.time()
                print("Autocorrelation time: {0:.2f} steps".format(sampler.get_autocorr_time(quiet=True)[0]))
    ...
    c.add_chain(samples, parameters=[r"$\alpha$",r"$\sigma_M$",r"$\mathcal{M}_{50}$",r"$\mathcal{B}$",
                                     r"$\mathcal{S}_{\mathrm{gal}}$",r"$\mathcal{A}$",r"$\sigma_{\log R}$",r"$n$"])
    ...
    fig = c.plotter.plot(figsize=(12,12),extents=[[-2,-1.1],[0,2.0],[5,10],[10**-5,3],[0.025,1.0],[10,500],[0,2.0],[0,2]],
    truth=[true_params['alpha'], true_params['sigma_M'], true_params['mpeak_cut']-np.log10(0.7), true_params['B'], 
           true_params['sigma_mpeak'], 1000.0*true_params['A'], true_params['sigma_r'], true_params['n']],
