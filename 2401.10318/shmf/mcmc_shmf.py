...
for i in [nhosts]:
    ...
    for move in moves:
        if MPI == True:
            with MPIPool() as pool:
                ...
                sampler = emcee.EnsembleSampler(nwalkers, ndim, posterior_shmf.lnpost_global_data, moves=move, pool=pool)
                start = time.time()
    ...
    c.add_chain(samples, parameters=[r"$\alpha$",r"$\sigma_M$",r"$\mathcal{M}_{50}$",r"$\mathcal{B}$",
                                     r"$\mathcal{S}_{\mathrm{gal}}$",r"$\mathcal{A}$",r"$\sigma_{\log R}$",r"$n$",r"$\xi_8$",r"$\xi_9$",r"$\xi_{10}$"])
    ...
    fig = c.plotter.plot(figsize=(12,12),extents=[[-2,-1.1],[0,2.0],[5,10],[10**-5,3],[0.025,1.0],[10,500],[0,2.0],[0,2],[-2,2],[-2,2],[-2,2]],
    truth=[true_params['alpha'], true_params['sigma_M'], true_params['mpeak_cut']-np.log10(0.7), true_params['B'], 
           true_params['sigma_mpeak'], 1000.0*true_params['A'], true_params['sigma_r'], true_params['n'], true_params['xi_8'], true_params['xi_9'], true_params['xi_10']],
